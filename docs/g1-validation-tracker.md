...
| S-10 | 2026-08-14 | Web + repo | Security re-check (Aug 14 20:15) | Re-check's remaining open items: (a) “S-7 verification — tampered/expired sig on api.magneetar.me/apk/download still returns the SPA page (200)”; (b) “repo git log secret-history scan” from the Agent Mode backlog; (c) “Play Store Accessibility justification for System Update Protection before submission” | (a) **Wrong-host probe.** The auditor pasted `magneetar.me/apk/download` (the marketing SPA host). That host has NO `/apk/download` route — unknown paths fall back to the landing page (200 text/html), which is benign (no binary ever streams from it). The signed link is api-host-only by design. Fresh cache-busted probe of the API host: `?expires=…&sig=deadbeef` → **403** `application/json` `{"detail":"Invalid or expired download link — mint a fresh one from the download page"}`. (b) **Secret-history scan CLEAN** — `git log --all -p -S` over secret-bearing patterns + the add-only filename scan found no real credentials ever committed: every env/secret file in history is a placeholder (`CHANGE_ME`, empty values, `.example` files); `mtk_*` appears only in docs/tests describing the prefix. (c) **Resolved by design** — `android-app/app/src/play/AndroidManifest.xml` removes `UninstallGuardService` via `tools:node="remove"` (comment: “Play Store rejects non-accessibility usage”); the merged Play manifest has ZERO accessibility matches (DISTRIBUTION_PLAN.md verified). The Play build ships no accessibility service, so no justification is required; the sideload build keeps it with a documented removal path | (a) No code change — behavior correct; re-verified live 2026-08-14 20:15+ (403 JSON). (b) No change. (c) Documented in play-store-checklist (new E.2 row) — gate item closed by flavor design | ✅ **RESOLVED (2026-08-14)** — (a) S-7 re-verified 403; (b) secret scan clean; (c) Play accessibility stripped by flavor, no justification needed |
