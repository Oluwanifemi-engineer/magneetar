...
| Armed Audio watch — VAD-gated STEALTH capture w/ 15s pre-roll + EVIDENCE escalation (game-changer gap-closer) | 🟡 IMPLEMENTED, awaiting on-phone field test (2026-08-15) |  |  |  |  |  | New ArmedAudioService (mic FGS): 16 kHz AudioRecord → VAD → 15s pre-roll ring → AAC segments (MediaCodec, WAV fallback) → SHA-256 hash-chained manifest → upload via existing /api/device/media. `capture_audio` routes to the watch when armed (mic exclusivity); else MediaCaptureService clip (unchanged). VAD design-doc bug fixed (G1-5). 5/5 VAD + 13/13 routing tests pass; both flavors built; APK staged — field test when phone reconnects |
