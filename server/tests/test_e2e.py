...
for mod_name in list(sys.modules.keys()):
    if mod_name.startswith("config") or mod_name.startswith("database") \
...
class TestDeviceManagement:
    ...
    def test_mark_device_recovered(self):
        ...
        headers = get_dash_headers()
        resp = client.post(
            "/api/dashboard/devices/e2e-test-device/recover",
            headers=headers
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_get_device_history(self):
        headers = get_dash_headers()
        resp = client.get(
            "/api/dashboard/devices/e2e-test-device/history",
            headers=headers
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_device_history_not_found(self):
        headers = get_dash_headers()
        resp = client.get(
            "/api/dashboard/devices/nonexistent-device/history",
            headers=headers
        )
...
class TestLiveLocation:
    def test_post_and_retrieve_location(self):
        ...
        resp = client.post(
            "/api/device/location",
            json={
                "device_id": device_id, "lat": 9.1000, "lng": 8.7000,
                "accuracy_horizontal": 15.0, "speed": 1.0,
                "provider": "gps", "ping_sequence": 1,
            },
            headers=dev_headers
        )
        ...
        resp = client.get(
            f"/api/dashboard/locations/{device_id}/live",
...
class TestTheftDetection:
    ...
    def test_theft_triggers_on_fresh_device(self):
        ...
        resp = client.post(
            "/api/device/location",
            json={
                "device_id": did, "lat": 9.25, "lng": 8.9,
                "accuracy_horizontal": 50.0, "speed": 45.0,
                "battery_percent": 60, "is_charging": False,
                "provider": "gps", "confidence_level": "MEDIUM",
                "is_location_enabled": False,
                "is_airplane_mode": True, "sim_changed": True,
                "ping_sequence": 1,
            },
            headers=dev_headers
        )
    ...
    def test_evidence_case_created_on_theft(self):
        ...
        resp = client.post(
            "/api/device/location",
            json={
                "device_id": did, "lat": 9.5, "lng": 8.5,
                "speed": 50.0, "sim_changed": True,
                "is_airplane_mode": True, "is_location_enabled": False,
                "ping_sequence": 1,
            },
            headers=dev_headers
        )
        ...
        dash_headers = get_dash_headers()
        resp = client.get(
            f"/api/dashboard/evidence/{did}", headers=dash_headers
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_normal_pings_no_theft(self):
        ...
        for seq in range(3):
            resp = client.post(
                "/api/device/location",
                json={
                    "device_id": did, "lat": 9.082, "lng": 8.6753,
                    "accuracy_horizontal": 8.0, "speed": 0.5,
                    "battery_percent": 85, "is_charging": False,
                    "provider": "gps", "confidence_level": "HIGH",
                    "is_location_enabled": True, "sim_changed": False,
                    "ping_sequence": seq + 1,
                },
                headers=dev_headers
            )
        ...
        for i in range(35):
            resp = client.post(
                ...
                json={
                    ...
                    "accuracy_horizontal": 10.0, "speed": 0.5,
                    "provider": "gps", "ping_sequence": i + 1,
                },
                headers=dev_headers
            )
...
class TestGeofenceBasic:
    ...
    def test_create_and_delete_geofence(self):
        ...
        resp = client.post(
            ...
            json={
                "device_id": self.DEVICE_ID, "name": "Test",
                "center_lat": 6.5, "center_lng": 3.4,
                "radius_meters": 500, "is_safe_zone": True,
            },
            headers=dash_headers
        )
        assert resp.status_code == 200
        ...
        # List
        resp = client.get(
            f"/api/dashboard/geofences/{self.DEVICE_ID}",
            headers=dash_headers
        )
        assert resp.status_code == 200
