...
from models import (
    DeviceRegistration, TelemetryPing, LocationReport, MediaReport,
    CommandRequest, CommandAck, HeartbeatPacket, OfflineQueueUpload,
    RefreshRequest, HealthResponse, ConfigResponse,
)
from auth import (
    create_device_tokens, hash_device_key, refresh_access_token,
    verify_api_key, get_current_device_or_key,
    check_location_rate_limit, check_media_rate_limit,
    check_command_poll_rate_limit, check_heartbeat_rate_limit,
)
...
async def post_location(
    ...
    db.execute(
        ...
        (
            ...
            report.gps_satellite_count, json.dumps(report.wifi_bssids or []),
            json.dumps(report.cell_tower_ids or []), report.ble_devices_nearby,
            ...
            score, threat_level, json.dumps(anomalies or []),
    ...
    db.execute(
        "UPDATE devices SET last_seen=?, sentinel_score=? WHERE id=?",
        (now, score, device_id)
    )
    db.commit()
