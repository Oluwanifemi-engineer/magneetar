...
from database import log_audit, check_rate_limit
...
def check_login_rate_limit(ip_address: str) -> bool:
    ...
    return check_rate_limit(
        f"login:{ip_address}",
        "login",
        settings.RATE_LOGIN_ATTEMPTS,
        settings.RATE_LOGIN_WINDOW_MINUTES
    )
...
def check_location_rate_limit(device_id: str) -> bool:
    ...
    return check_rate_limit(
        ...
        "location",
        30,  # 30 requests
        1  # per minute (effectively 1 per 2 seconds)
    )
...
def check_command_rate_limit(actor: str) -> bool:
    ...
    return check_rate_limit(
        f"command:{actor}",
        "command",
        settings.RATE_COMMAND_PER_MINUTE,
        1
    )
...
def check_media_rate_limit(device_id: str) -> bool:
    ...
    return check_rate_limit(
        f"media:{device_id}",
        "media",
        settings.RATE_MEDIA_PER_MINUTE,
        1
    )
...
def check_heartbeat_rate_limit(device_id: str) -> bool:
    ...
    return check_rate_limit(
        ...
        "heartbeat",
        settings.RATE_HEARTBEAT_PER_MINUTE,
        1
    )
...
def check_command_poll_rate_limit(device_id: str) -> bool:
    ...
    return check_rate_limit(
        f"command_poll:{device_id}",
        "command_poll",
        settings.RATE_COMMAND_POLL_PER_MINUTE,
        1
    )
...
def get_current_device_or_key(
    ...
    if x_device_key:
        ...
        with get_db_context() as conn:
            row = conn.execute(
                "SELECT id FROM devices WHERE device_key_hash=?",
                (key_hash,)
            ).fetchone()
            if row:
                return row["id"]
