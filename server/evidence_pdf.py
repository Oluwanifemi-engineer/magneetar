...
def _app_version() -> str:
    """Read the project version from the VERSION file (single source of
    truth, same as main.py). The old code read `settings.APP_VERSION`, which
    doesn't exist — every PDF footer hardcoded "v1.0.0" regardless of the
    actual release."""
...
class EvidencePDFGenerator:
    ...
    def generate(case_id: str) -> Optional[bytes]:
        ...
        elements.append(HRFlowable(width="100%", thickness=1, color=EvidencePDFGenerator.ACCENT, spaceAfter=4 * mm))
        ...
        elements.append(
            Paragraph(f'<font color="{status_color.hexval()}"><b>STATUS: {status}</b></font>', styles["Value"])
        )
        elements.append(Spacer(1, 3 * mm))

        # ── Device Information ─────────────────────────────────────────────
        elements.append(Paragraph("DEVICE INFORMATION", styles["SectionHeader"]))
        elements.append(HRFlowable(width="100%", thickness=0.5, color=EvidencePDFGenerator.BORDER, spaceAfter=2 * mm))
        ...
        if locations:
            ...
            if len(locations) > 50:
                elements.append(
                    Paragraph(
                        f"... and {len(locations) - 50} more location records (see dashboard)", styles["Monospace"]
                    )
                )
            elements.append(Spacer(1, 3 * mm))
        ...
        if alerts:
            ...
            for alert in alerts[:30]:
                ...
                elements.append(Paragraph(f"<b>{alert_time}</b> — {alert_type}", styles["AlertLine"]))
                if alert_msg:
        ...
        if chain:
            ...
            elements.append(
                Paragraph("SHA-256 Hash Chain — verifies integrity of all evidence items.", styles["Monospace"])
            )
            elements.append(Paragraph(f'<font face="Courier" size="6">{chain}</font>', styles["Monospace"]))
            elements.append(Spacer(1, 3 * mm))
            elements.append(
                Paragraph(
                    "<i>Verify this hash independently using the Magneetar dashboard.</i>",
                    ParagraphStyle(
                        "Disclaimer", fontName="Helvetica-Oblique", fontSize=7, textColor=colors.HexColor("#999999")
                    ),
                )
            )
        ...
        elements.append(
            Paragraph(
                f"This report was automatically generated by Magneetar v{_app_version()} "
                f"on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}.",
                ParagraphStyle("Footer", fontName="Helvetica", fontSize=6, textColor=colors.HexColor("#999999")),
            )
        )

        # Build the PDF
        doc.build(
            elements, onFirstPage=EvidencePDFGenerator._header_footer, onLaterPages=EvidencePDFGenerator._header_footer
        )
